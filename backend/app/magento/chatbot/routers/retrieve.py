...
def _build_answer_prompt(
    ...
    if purpose == "preamble":
        ...
        return (
            ...
            "Tone & length:\n"
            " - 3 to 5 sentences — rich and informative, drawing on the category "
            "description and the variety in the matched products. Substantive, "
            "not a flat one-liner.\n"
    ...
    if purpose == "category_info":
        ...
        return (
            ...
            "product example.\n\n"
            "Goal: write a thorough, informative overview of what the category "
            "covers, what's inside it, and (when you can tell) who it would "
            "suit — typically 4-7 sentences, more when the merchant's "
            "description is rich. Use that description as your source of truth "
            "and draw out its detail; paraphrase it naturally, don't quote "
            "verbatim, and don't leave useful information unused.\n\n"
    ...
    if purpose == "product_qa":
        ...
        return (
            ...
            "Goal: write a thorough answer that directly addresses the "
            "customer's question, drawing FULLY on the CMS content. Use all "
            "the relevant detail the sources provide — a clean yes/no can be "
            "2-3 sentences, but anything involved deserves a fuller answer "
            "(5-8 sentences, or a short structured explanation). Don't leave "
            "useful information from the sources unused. The customer will not "
            "see product cards beneath this reply, so the text must stand on "
            "its own.\n\n"
            "Rules:\n"
            " - Plain prose. No markdown headings. No bulleted or numbered "
            "lists unless the customer explicitly asked.\n"
    ...
    if purpose == "order_answer":
        ...
        return (
            ...
            " - Length matches the question: a status check is 1-2 sentences; "
            "\"what did I buy\" should walk through every item; a change "
            "request should fully explain the situation and the path forward. "
    ...
    if purpose == "purchase_history":
        ...
        return (
            ...
            " - 2-4 sentences, plain prose. The product list + one-tap "
    ...
    if active_retrieval and purpose == "answer":
        active_retrieval_rule = (
            " - **You have NO catalogue knowledge.** The sources below are just the product the customer is currently viewing (and, sometimes, the store's FAQ). You cannot see what ELSE the store carries without calling a tool. Therefore: any statement about the catalogue -- 'we do not list X', 'we do not carry Y', 'we don't currently have Z', 'we don't stock W', 'we don't offer V' -- MUST come AFTER a `find_products_listing` or `retrieve_more_products` call whose result showed no match for X/Y/Z/W/V. Making such a claim WITHOUT having run the tool is a fabrication, not an answer. If your first sentence would be 'we do not currently list [something]', STOP: call the tool for [something] first, and let the tool's result determine what you write. The tool call must cover the SPECIFIC thing you are about to claim -- not a broader one. If the customer asked about '20 GPM NOZZLES' and you only searched for '20 GPM', you cannot then claim 'no 20 GPM nozzles exist' -- you never checked nozzles specifically. Either narrow the search and run it again, or report only what the tool showed. And if the tool header says 'Showing X of Y' with Y greater than X, you have NOT seen the tail -- do not make claims about what is absent from it; either narrow the query and re-search, or report the truncation to the customer and offer to check specific items.\n"
            " - **Active Retrieval Tools.** The initial sources given to you contain ONLY the on-page product (and, when configured, FAQ entries). They cannot answer catalogue questions on their own. Any question about a spec this product does not have, or about other / similar / more / comparable products, requires a tool call BEFORE you write your answer -- not after, not conditional on your own judgment about whether the sources are 'sufficient'. Use `retrieve_more_content` for policy or help content, `find_products_listing` when the customer names a specific figure, and `retrieve_more_products` whenever the answer would be a LIST of comparable products -- 'what else do you have', 'other options in the same colour / size / material / brand / category', 'similar to this one', 'a cheaper version', 'more like this'. Do NOT ask the customer to narrow it down themselves, and do NOT stop with only the on-page product when they clearly asked for more. Only give up if, after executing your tool-calling step(s), the information remains unavailable.\n"
    ...
    return (
        ...
        "not answer a question about the one that was asked for. Answering around a gap without naming "
        "it is far worse than a short honest answer, because the customer cannot tell that you did not "
        "have it. This ordering outranks every instruction below about answering fully or at length.\n"
        " - Do NOT include phone numbers or email addresses in your reply — the "
        ...
        " - **Answer length: match the question and what the sources support.** A simple factual question "
        "(a price, a date, a yes/no) gets a tight 1-2 sentence answer. A question the sources cover in depth "
        "— policies, how-to, eligibility, anything multi-part — deserves a fuller answer: several sentences, "
        "or a short structured walk-through of the relevant points. Pull in the useful detail the sources "
        "provide (conditions, timeframes, exceptions, steps); don't pad with filler, but never leave out "
        "information the customer would find helpful just to be brief.\n"
        ...
        " - No markdown headings. Prefer plain prose, but you MAY use a short bulleted or numbered list when "
        ...
        + (
            "\n\n--- MANDATORY ---\n"
            "If the customer's question is about the catalogue -- naming a specific value, a "
            "product type, or using phrases like 'do you have', 'I'm looking for', 'other', "
            "'similar', 'more', 'what else' -- your NEXT output MUST be a tool call, not prose:\n"
            "  * compound value+type or browse-style question -> `retrieve_more_products(query=<full phrase>)`\n"
            "  * single specific value with no product type combined -> `find_products_listing(search_term=<value>)`\n"
            "  * policy / help / FAQ / warranty / shipping question -> `retrieve_more_content(query=<phrase>)`\n"
            "\n"
            "Forbidden first sentences (only allowed AFTER a tool call has returned no matches for X):\n"
            "  'we do not currently list X'   'we do not carry Y'   'we don't have Z'   'we don't stock W'\n"
            "Forbidden anywhere: 'would you like us to search?', 'shall I look?'.\n"
            "\n"
            "Example. Customer: 'I am looking for a 20 GPM flow-rate nozzle.'\n"
            "  Correct first action: call `retrieve_more_products(query='20 GPM nozzle')`.\n"
            "  WRONG first action: writing 'we do not currently list nozzles with 20 GPM' -- no tool has been called; that is fabrication.\n"
            "-----------------\n\n"
            if active_retrieval else "\n\n"
        )
        + f"Customer question: {query}"
        + history_block
        + f"\n\nSources:\n{sources_blob}"
